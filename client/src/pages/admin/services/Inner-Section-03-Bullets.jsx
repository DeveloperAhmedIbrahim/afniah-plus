...
const ServiceInnerSection03Bullets = () => {
  ...
  const fetchBullets = async () => {
    ...
    try {
        ...
        const res = await axiosInstance.get(`/admin/service/${id}/section-03/bullet/list?lang=${lang}`);
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editingItem
      ? `/admin/service/${id}/section-03/bullet/update/${editingItem.id}`
      : `/admin/service/${id}/section-03/bullet/insert`;
    ...
    try {
      ...
      if (!editingItem && response?.bullet) {
        ...
        navigate(`/admin/service/${id}/section-03/bullets/${response.bullet.id}?lang=ar`);
      } else if (editingItem) {
        await fetchBullets();
      }
    } catch (err) {
  ...
  const handleEdit = async (itemId) => {
    setFormLoading(true);
    try {
      const response = await axiosInstance.get(`/admin/service/${id}/section-03/bullet/update/${itemId}?lang=${lang}`);
  ...
  const handleCancel = () => {
    setEditingItem(null);
    navigate(`/admin/service/${id}/section-03/bullets?lang=${lang}`);
  };

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/admin/service/${id}/section-03/bullet/delete/${deleteId}`);
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
            <h1 className="text-2xl text-gray-700 flex items-center gap-2">
                Update <span className="text-green-primary font-semibold">{serviceTitle}</span> - Section 03 - Bullets
      ...
      <div className="flex justify-center">
        <Tabs value={lang} className="w-[400px]">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger
              value="en"
              onClick={() => navigate(id ? `/admin/service/${id}/section-03/bullets/${bulletId}?lang=en` : `/admin/service/${id}/section-03/bullets?lang=en`)}
            >
              English
            </TabsTrigger>
            <TabsTrigger
              value="ar"
              onClick={() => navigate(id ? `/admin/service/${id}/section-03/bullets/${bulletId}?lang=ar` : `/admin/service/${id}/section-03/bullets?lang=ar`)}
            >
              العربية
            </TabsTrigger>
          </TabsList>
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right space-x-1">
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => navigate(`/admin/service/${id}/section-03/bullets/${item.id}?lang=${lang}`)}
                          title='Edit'
                        >
...
export default ServiceInnerSection03Bullets;